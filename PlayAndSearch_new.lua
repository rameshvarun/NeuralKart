...
local search = loadfile("SearchAIBeam.lua")
...
while true do
  ...
  local ai_stuck_counter = 0
  local search_stuck_counter  = 0
  while util.readProgress() < 3 do
    ...
    if util.readProgress() > progress then
      ...
      ai_stuck_counter = ai_stuck_counter + 1
      if ai_stuck_counter > 2 then
